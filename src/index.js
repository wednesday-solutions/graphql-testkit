...
function createArgsAndBody(schema, entity, entityObj, result, variables, root, depth, variablesJSON = {}) {
  ...
  (entityObj?.fields || []).forEach(field => {
    ...
    if (
      !INVALID_TYPES.includes(field.type.kind)) {
...
function generateOperationOutput(schema, list, operationName, config) {
  ...
  list.forEach((e) => {
    ...
    if (!INVALID_TYPES.includes(entity.type.kind)) {
      ...
      let [result, variables, variablesJSON] = createArgsAndBody(schema, entity, entityObj, "", "", true, config.maxDepth);
      ...
      fs.writeFileSync(`output/${config.strippedEndpoint}/${operationName}/${entity.name}/${entity.type.name}.graphql`, result, {encoding: 'utf-8'})
      fs.writeFileSync(`output/${config.strippedEndpoint}/${operationName}/${entity.name}/variables.json`, JSON.stringify(variablesJSON), {encoding: 'utf-8'})
...
export const generateOutput = (config) => {
  ...
  fs.writeFileSync(`output/${config.strippedEndpoint}/collections.json`, JSON.stringify(collection), {encoding: 'utf-8'});
}